...
# Asumir que tenemos datos agregados por país
country_data <- filtered_data %>%
  group_by(cntry) %>%
  summarise(mean_responsibility = mean(as.numeric(ccrdprs_segmented)))

# Merge con datos geográficos
world <- left_join(world, country_data, by = c("iso_a2" = "cntry"))

# Crear el mapa
ggplot(data = world) +
  geom_sf(aes(fill = mean_responsibility)) +
